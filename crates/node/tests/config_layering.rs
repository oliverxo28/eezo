...
fn config_exposes_effective_cli() {
    ...
    let mut child = common::spawn_node_with(&[
        ...
        "--datadir", datadir,
        "--genesis", &format!("{}/../genesis.min.json", env!("CARGO_MANIFEST_DIR")),
    ]);
    ...
    assert_eq!(node.get("listen").and_then(|s| s.as_str()), Some("127.0.0.1:18082"));
...
fn config_reflects_log_level_cli() {
    ...
    let v: serde_json::Value =
        reqwest::blocking::get(&format!("http://127.0.0.1:{}/config", port)).unwrap().json().unwrap();
