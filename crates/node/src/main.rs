...
/// T62.0: GET /dag/block_preview
///
/// Returns a synthetic "block preview" from the current DAG candidate with decoded tx fields.
/// This does NOT commit anything; it only inspects the DAG + mempool.
#[cfg(feature = "pq44-runtime")]
async fn dag_block_preview_handler(State(state): State<AppState>) -> Response {
    if let Some(dag) = state.dag_runner.as_ref() {
        match dag.block_preview().await {
...
#[cfg(not(feature = "pq44-runtime"))]
async fn dag_block_preview_handler() -> Response {
    (
        StatusCode::SERVICE_UNAVAILABLE,
               ...
               // Build “block” index for the UI/debug (list of tx hashes as 0x…)
                ...
                for entry in batch {
                    ...
                    let from_hex = match parse_account_addr(&env.tx.from) {
                        ...
                        None => {
                            let reason = "invalid from address".to_string();
                            state_clone
                                .mempool
                                .mark_rejected(&entry.hash, reason.clone(), entry.bytes.len())
                                .await;
                            ...
                            // Skip to the next tx in this batch
                            continue;
                        }
                    };
                    ...
                    let to_hex = match parse_account_addr(&env.tx.to) {
                        Some(addr) => format!("0x{}", hex::encode(addr.as_bytes())),
                        None => {
                            let reason = "invalid to address".to_string();
                            state_clone
                                .mempool
                                .mark_rejected(&entry.hash, reason.clone(), entry.bytes.len())
                                .await;
